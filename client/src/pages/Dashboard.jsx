...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/builder-data", { withCredentials: true })
       .then((res) => {
  ...
  useEffect(() => {
    const loadTeam = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/load-team", { withCredentials: true });
  ...
  const saveTeam = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/save-team", { team: formatted }, { withCredentials: true });
    } catch (err) {
  ...
  const handleMoveChange = (i, j, move) => {
